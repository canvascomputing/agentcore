...
- Finish the ticket by calling `close_ticket` with `result` set to a JSON object of the form `{"idx": IDX, "partial_sum": N}`, copying `idx` verbatim from the task and using the integer the tool printed for `N`. Pass it as a JSON value, not a JSON-encoded string.
- NEVER add prose, code fences, or commentary outside the `close_ticket` call.
